...
def reviews():
    ...
    try:
        if product_from_db is None:
            ...
            if product_anchor_tag is not None:
                ...
                reviews_route = product_soup.find("div", "_3UAT2v _16PBlm")
                if reviews_route is not None:
                    reviews_route = reviews_route.parent.get("href")

                # removing all string after &aid including &aid
                if reviews_route is None:
                    raise Exception("Unable to scrape Reviews")
